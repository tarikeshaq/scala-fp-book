...
enum Free[F[_], A]:
  ...
  def run(using F: Monad[F]): F[A] = this.step match {
    ...
    case FlatMap(x, f) => x match {
      ...
      case _ => sys.error("Impossible, the step function handles those branches")
    }
  }
...
object Free:
 given freeMonad[F[_]]: Monad[[x] =>> Free[F, x]] with
  ...
  extension [A](fa: Free[F, A]) def flatMap[B](f: A => Free[F, B]): Free[F, B] = 
    fa.flatMap(f)

  extension [A](fa: Free[Function0, A])
    @annotation.tailrec
    final def runTrampoline: A = fa match { case Return(a) => a
      case Suspend(s) => s()
      case FlatMap(x, f) => x match {

...
object Tree:
  extension [A](t: Tree[A])
    def foldRight[B](acc: B)(f: (A, B) => B): B =
      ...
      t.tail.foldRight(next) { case (tNext, nextAcc) =>
        tNext.foldRight(nextAcc)(f)
      }
...
given monoidApplicative[M](using m: Monoid[M]): Applicative[[x] =>> Const[M, x]]
with
  ...
  given listTraverse: Traverse[List] with
    extension [A](fa: List[A])
      override def traverse[G[_]: Applicative, B](f: A => G[B]): G[List[B]] =
        fa.foldRight(summon[Applicative[G]].unit(List.empty)) { case (a, gl) =>
          f(a).map2(gl)(_ :: _)
        }
  ...
  given mapTraverse[K]: Traverse[[x] =>> Map[K, x]] with
    extension [V](m: Map[K, V])
      override def traverse[G[_]: Applicative, B](f: V => G[B]): G[Map[K, B]] =
        val g = summon[Applicative[G]]
        m.foldRight(g.unit(Map[K, B]())) { case ((k, v), acc) =>
          f(v).map2(acc) { case (b, innerM) =>
            innerM + (k -> b)
          }
        }
...
trait Applicative[F[_]] extends Functor[F]:
  ...
  def traverseM[K, V1, V2](m: Map[K, V1])(f: V1 => F[V2]): F[Map[K, V2]] =
    m.foldRight(unit(Map.empty)) { case ((k, v), acc) =>
      f(v).map2(acc) { case (v2, innerM) =>
        innerM + (k -> v2)
      }
    }
  ...
  def filterM[A](as: List[A])(f: A => F[Boolean]): F[List[A]] =
    as.foldRight(unit(List.empty)) { case (a, fl) =>
      f(a).map2(fl) {
        case (b, l) if b => a :: l
        case (_, l)      => l
      }
  ...
  def product[G[_]](G: Applicative[G]): Applicative[[x] =>> (F[x], G[x])] =
    ...
    val res: Applicative[[x] =>> (F[x], G[x])] = new:
      ...
      extension [A](fa: (F[A], G[A]))
        override def map2[B, C](
            fb: (F[B], G[B])
        )(f: (A, B) => C): (F[C], G[C]) = (fa, fb) match {
          case ((fa1, fa2), (fb1, fb2)) =>
            (F.map2(fa1)(fb1)(f), G.map2(fa2)(fb2)(f))
        }
    res
  ...
  extension [A](fa: F[A])
    ...
    def map4[B, C, D, E](fb: F[B], fc: F[C], fd: F[D])(
        f: (A, B, C, D) => E
    ): F[E] =
      val a = apply(unit(f.curried))(fa)
      val b = apply(a)(fb)
...
object ZipList:
  ...
  extension [A](zl: ZipList[A]) def foreach(f: A => Unit): Unit = zl.foreach(f)

...
object Par:
  ...
  extension [A](pa: Par[A])
    def run(e: ExecutorService): Future[A] = pa(e)  
  ...
  extension [A](pa: Par[A])
    def map2[B, C](b: Par[B])(f: (A, B) => C): Par[C] =
      e => new Future[C]:
        ...
        def isCancelled(): Boolean = cache.isEmpty && (futA.isCancelled() || futB.isCancelled())
  ...
  def parReduce[A, B](ps: IndexedSeq[A])(mapper: Option[A] => B)(join: (B, B) => B): Par[B] =
   fork:
...
def intParReducer: (IndexedSeq[Int], (Int, Int) => Int) => Par[Int] =
  (ints, join) =>
   Par.parReduce(ints)(numMapper)(join)
